 ...
 *  -------------------------------------------------------------------------
 *
 * Created: Mon Jun 08 15:48:99 2026
 */
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    ...
    if (outUnitIdReg != INVALID_UNIT_ID) {
      ...
      ssSetLocalErrorStatus(S,
                            "Invalid Unit provided for output port pos of S-Function sfcn_encoder");
      return;
    }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  const uint8_T *enc = (const uint8_T *) mxGetUint8s(PARAM_DEF0(S));
  const uint8_T *pinA = (const uint8_T *) mxGetUint8s(PARAM_DEF1(S));
  const uint8_T *pinB = (const uint8_T *) mxGetUint8s(PARAM_DEF2(S));
